...
class TexturedCube:
    def __init__(self, texture_path):
        ...
        self.create_single_texture(texture_path)

    def load_all_textures(self, img_folder, num_per_texture = 6):
        """返回所有纹理的列表"""
        for img_file in Path(img_folder).rglob("*.png"):
            texture_id = self.create_single_texture(str(img_file))
            self.texture_ids.append(texture_id)

        texture_pool = []

        for id in self.texture_ids:
            texture_pool.extend([id] * num_per_texture)
        random.shuffle(texture_pool)

        return texture_pool

    def create_single_texture(self, image_path):
        """创建一个纹理"""
        try:
...
def setup_lighting():
    ...
    diffuse = [light_diffuse, light_diffuse, light_diffuse, 1.0]  # 漫反射设为最亮！
...
def generate_dataset(num_per_texture=6):
    ...
    all_textures = list(base_dir.rglob("*.png"))
    if not all_textures: return print("未找到贴图")
    ...
    cube = TexturedCube(all_textures[0])
    texture_pool = cube.load_all_textures(base_dir, num_per_texture)
    process_flag = 0
    ...
    while process_flag < len(texture_pool):
        ...
        eye_y = random.uniform(3, 20)
        eye_z = random.uniform(-15, 5)
        gluLookAt(eye_x, eye_y, eye_z,
                  0, random.uniform(-3, 3), random.uniform(0, 5),
                  0, 1, 0)
        ...
        for j in range(len(position)):
            ...
            # 绘制KFS和地面
            cube.draw(texture_pool[process_flag], position[j])
            ...
            label_line = f"0 {xc:.6f} {yc:.6f} {bw:.6f} {bh:.6f}" # 写入bbox
...
if __name__ == "__main__":
    generate_dataset(num_per_texture=6* 6)  # 填入6的倍数（每张图六个KFS）